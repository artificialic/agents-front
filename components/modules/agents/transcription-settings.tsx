...
export function TranscriptionSettings({ agent, onAgentUpdate }: TranscriptionSettingsProps) {
  const [denoisingMode, setDenoisingMode] = useState<string>(agent.denoising_mode || 'noise-cancellation');
  const [transcriptionMode, setTranscriptionMode] = useState<string>(agent.stt_mode || 'fast');
  ...
  const handleDenoisingModeChange = async (value: string) => {
    try {
      ...
      await apiService.updateAgent(agent.agent_id, {
        denoising_mode: value as 'noise-cancellation' | 'noise-and-background-speech-cancellation'
      });
      await onAgentUpdate();
    } catch (error) {
  ...
  const handleTranscriptionModeChange = async (value: string) => {
    try {
      ...
      await apiService.updateAgent(agent.agent_id, {
        stt_mode: value as 'fast' | 'accurate'
      });
      await onAgentUpdate();
    } catch (error) {
    ...
    <TooltipProvider>
      <div className="flex flex-col items-start space-y-4 px-7 pb-4 pt-0">
        <div className="flex w-full flex-col rounded-lg pb-4 pl-4 pr-4 pt-4">
          ...
          <div className="text-xs text-muted-foreground">Filtra ruidos de fondo o conversaciones no deseadas.</div>
          <div className="mt-2">
                  ...
                  <div className="relative flex items-center">
                    <Tooltip delayDuration={0}>
                      ...
                      <TooltipContent side="top" className="max-w-xs">
                        <p className="text-xs">
                          Elimina el habla de fondo como voces humanas de un televisor, y la reducción de ruido agresiva
                          puede reducir la precisión de la transcripción en ciertos escenarios
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  </div>
        ...
        <div className="flex w-full flex-col rounded-lg pb-4 pl-4 pr-4 pt-4">
          ...
          <div className="text-xs text-muted-foreground">Balance entre velocidad y precisión.</div>
          <div className="mt-2">
                ...
                <div className="flex flex-1 flex-row items-center gap-2">
                  ...
                  <div className="relative flex items-center space-x-1">
                    ...
                    <Tooltip delayDuration={0}>
                      ...
                      <TooltipContent side="top" className="max-w-xs">
                        <p className="text-xs">
                          Funciona mejor con números y fechas al preservar un contexto más largo y evitar cortes de voz
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  </div>
        ...
        <div className="flex w-full flex-col rounded-lg p-4">
          ...
          <div className="mt-2 w-full">
            <Input
              ...
              placeholder="Separar por comas. Ejemplo: Walmart, Agente"
              className="w-full"
            />
          </div>
        </div>
